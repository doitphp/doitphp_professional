 ...
 * @version $Id: Template.class.php 2.0 2012-12-16 17:32:22Z tommy <streen003@gmail.com> $
 * @package core
 * @since 1.0
 */
...
class Template {
    ...
    public function __construct() {
        ...
        if (!$moduleName) {
            ...
            $viewDirName = 'modules' . DIRECTORY_SEPARATOR . $moduleName . DIRECTORY_SEPARATOR . 'views';
        }

        //设置当前视图的默认目录路径
        $this->_viewPath = BASE_PATH . $viewDirName . DIRECTORY_SEPARATOR;

        //设置视图编译缓存文件的默认目录路径
        $this->_compilePath = CACHE_PATH . 'views' . DIRECTORY_SEPARATOR;
    ...
    public function display($fileName = null) {
        ...
        //分析layout视图
        if ($this->_layout) {
            $layoutFile   = $this->_viewPath . 'layout' . DIRECTORY_SEPARATOR . $this->_layout . VIEW_EXT;
        ...
        if (!$layoutStatus) {
            ...
            //加载layout文件
            $layoutCompileFile = $this->_getCompileFile('layout' . DIRECTORY_SEPARATOR . $this->layout);
    ...
    protected function _parseViewName($fileName = null) {
        ...
        //参数分析
        if (!$fileName) {
            return Doit::getControllerName() . DIRECTORY_SEPARATOR . Doit::getActionName();
        }

        $fileName = str_replace('.', '/', $fileName);
        if (strpos($fileName, '/') === false) {
            $fileName = Doit::getControllerName() . DIRECTORY_SEPARATOR . $fileName;
        }
