 ...
 * @version $Id: File.class.php 2.0 2012-12-23 18:30:01Z tommy $
 * @package library
 * @since 1.0
 */
...
class File {
    ...
    public static function copyDir($source, $dest) {
        ...
        foreach ($fileList as $file) {
            if (is_dir($parseDir . DIRECTORY_SEPARATOR . $file)) {
                self::copyDir($parseDir . DIRECTORY_SEPARATOR . $file, $destDir . DIRECTORY_SEPARATOR . $file);
            } else {
                copy($parseDir . DIRECTORY_SEPARATOR . $file, $destDir . DIRECTORY_SEPARATOR . $file);
            }
        }
    ...
    public static function moveDir($source, $dest) {
        ...
        foreach ($fileList as $file) {
            if (is_dir($parseDir . DIRECTORY_SEPARATOR . $file)) {
                self::moveDir($parseDir . DIRECTORY_SEPARATOR . $file, $destDir . DIRECTORY_SEPARATOR . $file);
            } else {
                if (copy($parseDir . DIRECTORY_SEPARATOR . $file, $destDir . DIRECTORY_SEPARATOR . $file)) {
                    unlink($parseDir . DIRECTORY_SEPARATOR . $file);
                }
            }
        }
    ...
    public static function clearDir($dirName, $option = true) {
        ...
        foreach ($fileList as $file) {
            if (is_dir($parseDir . DIRECTORY_SEPARATOR . $file)) {
                self::clearDir($parseDir . DIRECTORY_SEPARATOR . $file, $option);
                if ($option == true) {
                    rmdir($parseDir . DIRECTORY_SEPARATOR . $file);
                }
            } else {
                unlink($parseDir . DIRECTORY_SEPARATOR . $file);
            }
